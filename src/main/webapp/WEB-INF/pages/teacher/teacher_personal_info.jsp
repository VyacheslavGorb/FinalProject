...
<div class="w-100 d-flex flex-column align-items-center">
    ...
    <c:if test="${sessionScope.is_teacher_info_error != null}">
    ...
    <form id="teacher-init-form" method="post" class="w-75" action="${pageContext.request.contextPath}/controller"
        ...
        <input type="hidden" name="command" value="update_teacher_description">
        <label class="form-label mt-4">
            <fmt:message key="teacher_init.label.description" bundle="${rb}"/>
        </label>
        <textarea id="description" name="description"
                  class="form-control" rows="5" maxlength="1000"
                  required>${requestScope.teacher.selfDescription}</textarea>

        <label class="form-label mt-4">
            <fmt:message key="teacher_init.label.experience" bundle="${rb}"/>
        </label>
        <input value="${requestScope.teacher.experienceYears}" id="experience" name="experience" type="text"
               pattern="[1-9]{1,2}" class="form-control" required>

        <label class="form-label mt-4">
            <fmt:message key="teacher_init.label.image" bundle="${rb}"/>
        </label>
        <input id="fileInput" name="image" type="file" class="form-control" accept="image/jpeg">

        <button id="submit-btn" type="submit" class="btn btn-success mt-4 mb-4 w-25">
            <fmt:message key="teacher_init.btn" bundle="${rb}"/>
        </button>
    </form>
</div>
...
<script src="script/teacher_personal_info.js"></script>
</html>
